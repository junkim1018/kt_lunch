...
export default function LunchRecommender() {
  ...
  const shuffleQuickPresets = () => {
    ...
    // 프리셋의 날씨를 현재 날씨로 덮어쓰기 (날씨가 컨셉인 프리셋은 weatherFixed로 보호)
    const adjusted = QUICK_PRESETS.map(p => {
      if (p.weatherFixed) return p;
    ...
    const timeout = setTimeout(async () => {
      try {
        ...
        const matchMood = (r) => {
          ...
          // 해장: mood에 'hangover'가 있거나 카테고리에 국밥/해장/탕 등 포함 (부적절한 카테고리 제외)
          if (selections.mood === 'hangover') {
            const category = r.category || '';
            // 1단계: 해장에 부적절한 카테고리 강제 제외
        ...
        const withMatches = restaurantDB.map(r => {
          ...
          // 🍜 해장/임원 등 중요 mood 미매칭 시 강한 감점
          if (['hangover', 'executive'].includes(selections.mood) && !matches[1]) {
            totalScore -= 25; // mood가 핵심인 상황에서 미매칭이면 대폭 감점
          }
        ...
        // 채식 식당(10개뿐)은 식단 선택과 무관하게 예산/인원 필터 면제
        const isVegetarianRestaurant = (r) => r.diet && Array.isArray(r.diet) && r.diet.includes('vegetarian');
        const candidates = withMatches.filter(r => {
          const vegPass = isVegetarianRestaurant(r);
